...
def generalised_expected_utility_loss(front, reference_set, utility_fns):
    """Compute the expected utility loss for a front and utility functions wrt a reference set."""
    utility_losses = []
    for utility_fn in utility_fns:
        front_utilities = utility_fn(reference_set)  # Compute the utility for the front
        approx_utilities = utility_fn(front)  # Compute the utility for the approximate front
        utility_loss = np.max(front_utilities) - np.max(approx_utilities)  # Compute the utility loss.
        utility_losses.append(utility_loss)
    return np.mean(utility_losses)
